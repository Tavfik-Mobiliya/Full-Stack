...
describe("Validation Schemas", () => {
  ...
  describe("testimonialSchema", () => {
    ...
    it("should fail with rating out of range [1, 5]", () => {
      const tooLow = testimonialSchema.safeParse({ ...validTestimonial, rating: 0 });
      const tooHigh = testimonialSchema.safeParse({ ...validTestimonial, rating: 6 });
  ...
  describe("productCreateSchema", () => {
    ...
    it("should succeed with minimal required fields and set default images array", () => {
      const result = productCreateSchema.safeParse(validProject);
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    it("should fail if year is less than 1900 or greater than 3000", () => {
      const tooLow = productCreateSchema.safeParse({ ...validProject, year: 1899 });
      const tooHigh = productCreateSchema.safeParse({ ...validProject, year: 3001 });
      expect(tooLow.success).toBe(false);
      expect(tooHigh.success).toBe(false);
    });

    it("should fail if collectionId is non-positive or not integer", () => {
      const negative = productCreateSchema.safeParse({ ...validProject, collectionId: -1 });
      const zero = productCreateSchema.safeParse({ ...validProject, collectionId: 0 });

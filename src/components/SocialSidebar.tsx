...
export const SocialSidebar: React.FC = () => {
  ...
  const handleWhatsAppRedirect = () => {
    ...
    const text = encodeURIComponent(
      "Hello Tevfik Mobilya, I would like to inquire about your luxury design services."
    );
    window.open(`https://wa.me/${waNumber}?text=${text}`, "_blank");
  };

  return (
    <aside className="fixed right-0 top-1/2 -translate-y-1/2 rounded-l-xl border-l border-y border-outline-variant shadow-2xl bg-surface-container-lowest/95 backdrop-blur-2xl flex flex-col gap-2 p-2 z-50 hover:-translate-x-1 transition-transform duration-300">

      {/* WhatsApp */}
      <button
        onClick={handleWhatsAppRedirect}
        className="p-3 text-on-surface-variant hover:text-[#25D366] hover:bg-[#25D366]/10 transition-all rounded-lg group relative cursor-pointer"
        title="WhatsApp"
      >
      ...
      <a
        ...
        className="p-3 text-on-surface-variant hover:text-[#E1306C] hover:bg-[#E1306C]/10 transition-all rounded-lg group relative flex items-center justify-center"
        title="Instagram"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
